...
while (True):
    try:
        ...
        text_output = inference(cropped_numbers)
        compressed_img = compress_image(img_full)

        # replace Adafruit IO user with your Adafruit IO user
        client.publish("igeorge/feeds/reading", text_output)
        client.publish("igeorge/feeds/unit", str(text_output))
        client.publish("igeorge/feeds/openmv", compressed_img)
